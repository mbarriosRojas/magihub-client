    ...
    <TooltipAnchor
      ...
      render={
        ...
        >
          {isProduction ? (
            <>
              <span className="slow-pulse size-1.5 rounded-full bg-orange-500" />
              <span>{label}</span>
            </>
          ) : (
            <>
...
const getTimelineConnectorClasses = (isSelected: boolean, isProduction: boolean) => {
  ...
  if (isProduction) {
    return 'border-orange-400 bg-transparent text-orange-500';
  }
        ...
        <div className="flex items-center justify-between gap-2">
          <span
            className={cn(
              ...
              isSelected ? 'text-orange-700 dark:text-orange-400' : 'text-text-primary',
            )}
          >
